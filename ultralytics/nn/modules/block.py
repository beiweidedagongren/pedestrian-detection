...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, dim, n_win=3, num_heads=8, qk_dim=256, qk_scale=0.1,
                 kv_per_win=8, kv_downsample_ratio=2, kv_downsample_kernel=(3,3), kv_downsample_mode='ada_maxpool',
                 topk=3, param_attention="qkvo", param_routing=True, diff_routing=True, soft_routing=True,
                 side_dwconv=3,
                 auto_pad=True):
        super().__init__()
        self.dim = dim
        ...
        self.lepe = nn.Conv2d(dim, dim, kernel_size=side_dwconv, stride=1, padding=(side_dwconv-1) // 2,
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        if self.auto_pad:
            ...
            pad_l = pad_t = 0
            pad_r = ((self.n_win - W_in % self.n_win) % self.n_win) if W_in % self.n_win != 0 else 0
            pad_b = ((self.n_win - H_in % self.n_win) % self.n_win) if H_in % self.n_win != 0 else 0
            x = F.pad(x, (0, 0,  # dim=-1
                          pad_l, pad_r,  # dim=-2
                          pad_t, pad_b))  # dim=-3
