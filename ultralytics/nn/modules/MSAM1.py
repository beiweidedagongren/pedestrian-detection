...
class EnhancedSpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv3 = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)
    ...
    def forward(self, x):
        ...
        combined = torch.cat([avg_out, max_out], 1)

        conv_out1 = self.conv1(combined)
        conv_out2 = self.conv2(combined)
        conv_out3 = self.conv3(combined)

        combined_conv = conv_out1 + conv_out2 + conv_out3

        attention = self.act(combined_conv)
        return x * attention
