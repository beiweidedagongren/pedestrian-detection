...
def compute_IoG(gt_box, pred_box):
...
def compute_elliptical_IoG(gt_box, pred_box):
    inter_xmin = torch.max(gt_box[:, 0], pred_box[:, 0])
    inter_ymin = torch.max(gt_box[:, 1], pred_box[:, 1])
    inter_xmax = torch.min(gt_box[:, 2], pred_box[:, 2])
    inter_ymax = torch.min(gt_box[:, 3], pred_box[:, 3])
    ...
    G = (np.pi / 4 * w * h).clamp(1e-6)
    return I / G

def repulsion_loss(pbox, gtbox, fg_mask, sigma_repgt=0.9, sigma_repbox=0.5, pnms=0.5, gtnms=0.5):
    ...
    for idx in range(pbox.shape[0]):
        ...
        if num_repgt > 0:
            ...
            IoG_value = compute_elliptical_IoG(gtbox_sec, pbox_sec)
            loss_repgt += smooth_ln(IoG_value, sigma_repgt).mean()
